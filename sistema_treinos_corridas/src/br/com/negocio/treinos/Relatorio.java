...
public class Relatorio {
    ...
    public static void exportarRelatorioExcelLindo(Usuario cliente, String caminhoArquivo) throws IOException {
        ...
        try (PrintWriter writer = new PrintWriter(new FileWriter(caminhoArquivo, StandardCharsets.UTF_8))) {
            ...
            writer.println("  <Style ss:ID=\"sHeader\"><Font ss:FontName=\"Calibri\" ss:Size=\"12\" ss:Bold=\"1\" ss:Color=\"#000000\"/><Interior ss:Color=\"#4AFF56\" ss:Pattern=\"Solid\"/><Borders><Border ss:Position=\"Bottom\" ss:LineStyle=\"Continuous\" ss:Weight=\"1\"/></Borders><Alignment ss:Horizontal=\"Center\"/></Style>");
            ...
            for (Treino t : cliente.getTreinos()) {
                ...
                writer.println("    <Cell ss:StyleID=\"sCenter\"><Data ss:Type=\"Number\">" + (t.getDuracaoSegundos()/60) + "</Data></Cell>");
                writer.println("    <Cell ss:StyleID=\"sCenter\"><Data ss:Type=\"Number\">" + String.format(Locale.US, "%.2f", dist) + "</Data></Cell>");
                writer.println("    <Cell ss:StyleID=\"sCenter\"><Data ss:Type=\"Number\">" + String.format(Locale.US, "%.0f", t.calcularCaloriasQueimadas(cliente)) + "</Data></Cell>");
                writer.println("   </Row>");
            }
            writer.println("  </Table>");
    ...
    public static String gerarRankingDesafioTexto(Desafio desafio) {
        ...
        if (participacoes.isEmpty()) return "Ainda não há participantes neste desafio.\n";
        for (ParticipacaoDesafio p : participacoes) p.setProgresso(calcularProgressoDesafio(p.getUsuario(), desafio));
        ...
        for (ParticipacaoDesafio p : participacoes) {
            sb.append(pos).append("º Lugar - ").append(p.getUsuario().getNome()).append(": ").append(String.format("%.2f km", p.getProgresso() / 1000.0)).append("\n");
            pos++;
        }
        return sb.toString();
    }
    private static double calcularProgressoDesafio(Usuario usuario, Desafio desafio) {
        ...
        for (Treino treino : usuario.getTreinos()) {
            if (treino instanceof Corrida) {
                ...
                if (!dataC.isBefore(desafio.getDataInicio()) && !dataC.isAfter(desafio.getDataFim())) progressoTotal += c.getDistanciaEmMetros();
            }
        }
